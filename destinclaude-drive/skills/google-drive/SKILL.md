...
Before proceeding, check rclone is configured:
- If `rclone config show gdrive` fails or shows no config:
  1. Guide the user through `rclone config` to create a remote named `gdrive` (type: drive)
  2. Verify with `rclone lsd gdrive:` before proceeding
