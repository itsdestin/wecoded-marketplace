...
- If `~/.claude/inbox/` does not exist:
  1. Create `~/.claude/inbox/`, `~/.claude/inbox/screenshots/`, `~/.claude/inbox/rants/`, `~/.claude/inbox/journal-queue/`, `~/.claude/inbox/processed/`
  2. Write `~/.claude/inbox/.config.json` with `{"providers": {}}` — providers (Todoist, Drive, Gmail) are configured on first use
