...
Before proceeding, check if this package has been initialized:
- If `~/.claude/encyclopedia/` does not exist:
  1. Ask the user's name and store it in `~/.claude/encyclopedia/.config.json` as `{"name": "..."}`
  2. Copy all files from this skill's sibling `templates/` directory (i.e., `~/.claude/plugins/destinclaude-encyclopedia/templates/*.md`) into `~/.claude/encyclopedia/`
  3. Confirm setup is complete, then proceed with the main workflow
