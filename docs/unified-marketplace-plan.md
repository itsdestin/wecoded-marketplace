...
- **Android plugin install routing gap.** `SessionService.kt` handles plugin installs for Anthropic-sourced entries by calling `PluginInstaller` directly (lines 436-471). But DestinClaude-sourced plugins (`sourceMarketplace == "destinclaude"`) fall through to `LocalSkillProvider.install()`, which throws for all plugin types. This means DestinClaude plugins and any future "destincode"-sourced packages can't install on Android. The fix is to consolidate all install routing into `LocalSkillProvider` so SessionService is a thin dispatcher.
...
- **`sync.js` silently removes delisted entries.** It rebuilds the entire non-DestinClaude section from scratch each run. If Anthropic delists a plugin, it simply vanishes from the next sync output — no deprecation flag, no warning. Users who have the plugin installed lose visibility of it in the marketplace.
...
- **Theme installs are filesystem-only.** No central manifest tracks installed themes — the app discovers them by scanning `~/.claude/destinclaude-themes/*/manifest.json`. No version tracking, no update detection.
...
**Decision:** Install artifacts where the runtime expects them. Track everything in the existing `destincode-skills.json` manifest.
...
- The existing `~/.claude/destincode-skills.json` is extended with a `packages` field to track what the marketplace installed, with version, source, and component paths
...
```jsonc
// ~/.claude/destincode-skills.json (extended)
{
  "version": 2,
  "favorites": [...],
  "chips": [...],
...
|--------|--------|-----------|------|---------|-------|
| `"marketplace"` | Yes | Yes | No | No | DestinCode / Anthropic / Community |
...
**Reconciliation:** On marketplace open, the app verifies that tracked component paths exist on disk. Missing paths are marked broken. Untracked directories in `~/.claude/plugins/` and `~/.claude/destinclaude-themes/` are surfaced as "External" items. This prevents manifest/filesystem drift.

### 2. The marketplace repo becomes a plugin host

**Decision:** `itsdestin/destincode-marketplace` gains a `marketplace.json` and `plugins/` directory, mirroring Anthropic's repo structure. The theme registry moves here from `destinclaude-themes`.
...
|   +-- destinclaude-life/
|   +-- destinclaude-productivity/
|   +-- some-community-plugin/
|   +-- ...
+-- skills/
|   +-- index.json                <- Generated: all skills + plugins from both sources
+-- themes/
|   +-- index.json                <- Generated: all themes (moved from destinclaude-themes)
...
**Two-repo cache for `local` source plugins:** The installer needs to know which repo to clone based on `sourceMarketplace` (or a `sourceRepo` field in the index entry). Anthropic `local` plugins clone from `anthropics/claude-plugins-official`. DestinCode `local` plugins clone from `itsdestin/destincode-marketplace`.
...
- Plugins discover config via environment variable `DESTINCODE_CONFIG_DIR` (set to `~/.claude/destincode-config/`) — plugins read `$DESTINCODE_CONFIG_DIR/<their-own-id>.json`
...
**Exception:** Plugins imported from Anthropic that use Claude Code's native `${CLAUDE_PLUGIN_ROOT}/config.json` pattern keep their config where Claude Code puts it. The DestinCode config system only manages plugins that declare `configSchema` in their marketplace entry.
...
- Core is tracked in `destincode-skills.json` `packages` with `"removable": false`
...
- The setup wizard checks `packages` in `destincode-skills.json` and skips layers already installed via the marketplace
...
The sync script needs to detect changes between runs instead of treating every run as a fresh import. Today it rebuilds all non-DestinClaude entries from scratch, silently removing any entries that Anthropic has delisted.
...
- Move theme registry from `destinclaude-themes/registry/theme-registry.json` to `themes/index.json`
...
- Loads `packages` from `destincode-skills.json`
...
- Record in `destincode-skills.json` `packages` with version, source, component paths
...
- Update installer to be source-aware for local plugins: clone `anthropics/claude-plugins-official` for Anthropic locals, clone `itsdestin/destincode-marketplace` for DestinCode locals
...
- Update the version in `destincode-skills.json` `packages`
...
- Size limit
- ID uniqueness across both DestinCode and Anthropic sources
...
Add `theme:marketplace:install` and `theme:marketplace:uninstall` bridge messages. Download manifest to `~/.claude/destinclaude-themes/`. Apply themes that only use color tokens and layout settings — no custom assets. Many themes work with just tokens.

#### 5c. Custom theme assets

Implement `shouldInterceptRequest()` in `WebViewHost.kt`'s `WebViewClient` to intercept `theme-asset://` URLs. Read the file from `~/.claude/destinclaude-themes/<slug>/<path>` and return a `WebResourceResponse` with the bytes and correct MIME type. This is the direct Kotlin equivalent of Electron's `protocol.handle()` in `theme-protocol.ts`. Unlocks background images, particle SVGs, mascots, custom CSS.

#### 5d. Theme hot-reload

Add a `FileObserver` on the `~/.claude/destinclaude-themes/` directory. On file changes, send reload event via bridge WebSocket. Nice-to-have, not blocking.

### Phase 6: Migration

#### 6a. Migrate existing installs

- Detect existing toolkit layer installs (symlinks in `~/.claude/plugins/` pointing into `destinclaude/`, or `installed_layers` in toolkit `config.json`) and create entries in `destincode-skills.json` `packages`
- Convert `destincode-skills.json` version 1 `installed_plugins` to version 2 `packages` entries (Phase 1d migration logic)
...
Users can install plugins via Claude Code's `/plugin install` (writes to `installed_plugins.json`) or via DestinCode's marketplace (writes to `destincode-skills.json`). These can diverge.

**Approach:** On marketplace open, run filesystem reconciliation. Scan `~/.claude/plugins/` and `~/.claude/destinclaude-themes/` for all installed items. Show DestinCode-managed ones with full manage actions. Show untracked items as "External" (visible, not manageable). Mark tracked items whose paths no longer exist as "broken" with a cleanup option. This is how VS Code handles extensions installed via CLI vs GUI.

### The 47 `local` Anthropic plugins

These are the only plugins where Anthropic's repo is in the file transfer path. Installing one requires cloning `anthropics/claude-plugins-official` to the cache directory (`~/.claude/destincode-marketplace-cache/`). With the new repo structure, DestinCode `local` plugins also need a cache clone of `itsdestin/destincode-marketplace`. The installer must be source-aware — this is explicitly handled in Phase 3a.

### `sync.js` entry removal

When Anthropic delists a plugin, `sync.js` currently silently removes it from the next output (it rebuilds all non-DestinClaude entries from scratch). The improved sync (Phase 1a) diffs against the previous output and flags removed entries with `"deprecated": true` rather than deleting them, since users may have them installed.
...
3. **Extend `destincode-skills.json`** — Add `packages` field, write migration from v1. The tracking manifest, needed before UI work.
