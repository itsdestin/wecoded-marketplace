...
We're unifying DestinCode's three separate content systems (skill marketplace, theme marketplace, and DestinClaude toolkit layers) into a single marketplace. Two documents in `destincode-marketplace/docs/` contain the full plan:
...
Android (destincode/app/src/main/kotlin/com/destin/code/):
...
- `destincode-marketplace/scripts/sync.js` — how upstream entries are imported
- `destincode-marketplace/index.json` — current schema (sample a few entries of different types)

Toolkit:
- `destinclaude/core/plugin.json` — Core plugin declaration
- `destinclaude/life/plugin.json` and `destinclaude/productivity/plugin.json` — layer declarations
