...
**Date:** 2026-04-07
**Context:** Research session exploring how to unify DestinCode's skill marketplace, theme marketplace, and DestinClaude toolkit layer system into a single marketplace with a visual UI and full install/update/remove lifecycle.

---

## Part 1: What We Investigated

This session set out to understand how plugins, skills, themes, and toolkit features are installed, updated, and managed across the DestinCode ecosystem — with the goal of designing a single unified marketplace to replace the current fragmented systems.

We investigated:
- The native Claude Code plugin marketplace (`anthropics/claude-plugins-official`)
- The DestinCode skill marketplace (`itsdestin/destincode-marketplace`)
- The DestinCode theme marketplace (`itsdestin/destinclaude-themes`)
- The DestinClaude toolkit layer system (core/life/productivity)
- The hook relay system between the DestinCode app and Claude Code
...
### 2a. The DestinCode Skill Marketplace (Current State)

**Registry:** `destincode-marketplace/index.json` — 151 entries (29 DestinClaude + 122 imported from Anthropic)

**Import pipeline:** `scripts/sync.js` reads Anthropic's `marketplace.json`, transforms entries to DestinCode's schema, and writes to `index.json`. DestinClaude entries (marked `sourceMarketplace: "destinclaude"`) are protected from overwrite. The script does NOT preserve SHA hashes from upstream — it assigns a static `"1.0.0"` version and a `publishedAt` timestamp to every imported entry.

**App fetches from:** `raw.githubusercontent.com/itsdestin/destincode-marketplace/main/index.json` with 24-hour cache TTL. Cached to `~/.claude/destincode-marketplace-cache/` (which exists but is currently empty on Destin's machine — the cache is populated lazily on first "local" source plugin install).

**Install flow (desktop):**
1. `skill-provider.ts` fetches the index, finds the entry
2. If type is `prompt` → stores directly in `destincode-skills.json` as a private skill (no files on disk)
...
5. Records install metadata in `destincode-skills.json` under `installed_plugins`
...
**Current state on Destin's machine:** Zero plugins installed via the marketplace. The `destincode-skills.json` file has favorites and chips but no `installed_plugins` section. The marketplace cache directory is empty.

**Uninstall:** Deletes the plugin directory from `~/.claude/plugins/` and removes the entry from `destincode-skills.json`. Cascade cleanup removes favorites, chips, and overrides for that ID.

**No update detection.** No SHA pinning, no version comparison. The only update mechanism is the DestinClaude `/update` command's `phase_marketplace_plugins`, which re-copies/re-pulls installed plugins.
...
### 2b. The DestinCode Theme Marketplace (Current State)

**Registry:** `destinclaude-themes/registry/theme-registry.json` — auto-generated from `themes/{slug}/manifest.json` files.
...
**Install:** Downloads manifest + assets to `~/.claude/destinclaude-themes/{slug}/`. Validates CSS safety for community themes (no @import, no external URLs, no expressions). Enforces 10MB total size limit.

**Publish flow (fully implemented):** Fork `itsdestin/destinclaude-themes` via `gh` CLI → create branch `theme/{slug}` → generate preview PNG via offscreen Electron window → upload files via GitHub Contents API → open PR. CI validates token requirements, CSS safety, size limits, slug uniqueness. Auto-rebuilds registry on merge.
...
### 2d. The DestinClaude Toolkit Layer System
...
**Purpose:** The DestinCode desktop app needs to know about Claude Code hook events (especially PermissionRequest) to display them in the React UI.
...
**Preservation:** The script finds existing relay entries by command path and updates them in place. All other hooks (DestinClaude's, user's custom, plugin hooks) are left untouched. Additive only.
...
**Wrong.** DestinCode has a full plugin installation pipeline — `PluginInstaller` on both platforms handles git cloning, sparse checkout, file copying, plugin.json normalization, conflict detection against native installs, and install metadata tracking.

### 3c. "The relay system might conflict with plugin hooks"
**Wrong.** The relay is additive — it doesn't overwrite or block other hooks. `install-hooks.js` carefully searches for existing relay entries and updates only those. DestinClaude hooks, native plugin hooks, and relay hooks all coexist without conflict.
...
**Wrong.** It's a flat archive from Google Cloud Storage. The `.gcs-sha` file is an integrity hash, not a git commit SHA. DestinCode's cache IS a git clone (separate thing).

### 3f. Overstating the "dual registry" problem
Presented as a major issue, but in reality zero plugins have been installed via DestinCode's marketplace. The `installed_plugins` section doesn't even exist in `destincode-skills.json` yet. The dual registry problem is theoretical at this point.
...
We traced the Kotlin `PluginInstaller.kt` and `LocalSkillProvider.kt`, but never verified whether plugin installation actually works on Android. The `install()` method in `LocalSkillProvider` throws "Plugin installation from marketplace not yet implemented" for non-DestinClaude plugins. Current state of Android marketplace functionality is unclear.

### 4c. How `sync.js` handles entry removal
We know it protects DestinClaude entries and imports upstream entries. But what happens when an upstream plugin is removed from Anthropic's marketplace? Does sync.js delete the entry from our index? Leave it as an orphan? Never checked.

### 4d. The SkillScanner internals
We know it scans `~/.claude/plugins/` for installed skills, but never read `skill-scanner.ts` in detail. Don't know how it handles edge cases like malformed plugin.json, missing SKILL.md files, or the interaction between native plugins and DestinCode-installed plugins.
...
Settled on packages installing to `~/.claude/destincode-packages/<id>/` with a `plugin/` subdirectory. But didn't definitively decide whether the app should symlink or copy. Symlinks are cleaner but fragile on Windows. Copies are reliable but mean the config.json trick wouldn't work if the copy is separate from the package.

### 4i. Whether Claude Code's auto-discovery handles symlinked plugin directories
If `~/.claude/plugins/my-plugin` is a symlink to `~/.claude/destincode-packages/my-plugin/plugin/`, does Claude Code's scanner follow the symlink? Likely yes on macOS/Linux. Uncertain on Windows (junctions vs symlinks vs MSYS symlinks).

### 4j. Theme registry migration
The current theme registry lives in `destinclaude-themes/registry/theme-registry.json`. In the unified model, it moves to `destincode-marketplace/themes/index.json`. Never discussed the migration path — how to move the registry, update the fetch URLs, handle users on old app versions, etc.

### 4k. Backwards compatibility
If users have the current system (separate skill/theme marketplaces, toolkit layers via setup wizard), what happens when they update to the unified marketplace? Do existing installs get migrated? Does the old `destincode-skills.json` format get preserved? What about users who installed layers via the setup wizard — do those become "installed packages" retroactively?
...
One marketplace in the DestinCode app with two browsable sections: "Skills & Plugins" and "Themes." Toolkit add-ons appear as featured/curated entries within Skills & Plugins, not as a separate section. They're just "bigger" skills from the user's perspective.

**2. DestinCode Marketplace is the sole source of truth.**
The app only talks to one registry: `itsdestin/destincode-marketplace`. This registry aggregates entries from Anthropic's marketplace (via `sync.js`), DestinClaude toolkit components, and community contributions. Once an entry is in our marketplace, it's ours — the app doesn't interact with upstream sources at runtime.
...
**4. Packages install to `~/.claude/destincode-packages/<id>/`.**
Each package is a self-contained directory:
```
~/.claude/destincode-packages/<id>/
...
**7. Updates flow through the DestinCode Marketplace.**
...
**New infrastructure:**
- `~/.claude/destincode-packages/` directory and package manager in the app
...
- `plugin-installer.ts` → install to `destincode-packages/<id>/` instead of `plugins/<id>/`, then handle the `plugin/` → `~/.claude/plugins/` step
...
- Move from `destincode-skills.json`'s `installed_plugins` to package manifests
...
- Create `themes/index.json` (import from `destinclaude-themes` registry)
...
**Highest risk:** Merging Core into the app. This is the most complex and least well-defined part of the plan. Core provides critical session infrastructure (sync, file guards) that currently runs as shell hooks. Replicating this in the app means the app must be running for these features to work — but users can run Claude Code without the DestinCode app open.

**Medium risk:** Windows symlink reliability. The package system relies on `~/.claude/plugins/<id>` pointing to `~/.claude/destincode-packages/<id>/plugin/`. Windows symlinks are fragile (require Developer Mode, MSYS configuration, and the correct symlink type). A fallback to file copying would break the `config.json` trick.
